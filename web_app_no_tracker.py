...
def upload_file():
    ...
    if file_extension in ['.jpg', '.jpeg', '.png']:
        ...
        img_cv2, counts, tracked_ids = draw_results_on_image(img_cv2, results, counts, tracked_ids)
        ...
        while cap.isOpened():
            ...
            frame_with_results, counts, tracked_ids = draw_results_on_image(frame, results, counts, tracked_ids)
...
def draw_results_on_image(img_cv2, results, counts, tracked_ids):
    ...
    for box in detections:
        ...
        if tracking_id is not None:
            tracked_ids.add(tracking_id)  # Track unique object IDs

        # Debugging to check if the ID is being assigned
        print(f"Object detected: {class_names[class_index]}, ID: {tracking_id}, Confidence: {confidence}")
        ...
        if tracking_id is not None:
            label_with_conf += f" ID: {tracking_id}"  # Add the tracking ID to the label if it exists
        
        cv2.rectangle(img_cv2, (x1, y1), (x2, y2), color, 2)
        cv2.putText(img_cv2, f"{label_with_conf}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)

        # Increment the count for the class only if a unique tracking ID is found
        if tracking_id is not None and tracking_id not in tracked_ids:
